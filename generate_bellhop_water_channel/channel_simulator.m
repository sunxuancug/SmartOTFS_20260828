...
function channel_simulator(channel_name)
tstart= tic;
cd(fileparts(which('channel_simulator')));
path(genpath([pwd,'/BellhopFunctions']), path);
...
case 'bel'
    [lmean,taumean,theta,ns,nb,hp,p0_ind]=runBellhop(h,h-ht,h-hr,d,fc,c,c2,file_name);
...
sig_delp= sqrt(double(1/c^2*((2*sin(theta)).^2)).*double(ns*sig2s+nb*sig2b)); 
...
vvhcl= vtv.*cos(theta(1)-theta_tv)-vrv.*cos(theta(1)+theta_rv); %%%
%vvhcl= 0;
avhcl= vvhcl/c;
...
adopp= double(adrift) + double(avhcl) + double(asurf*ns(1)); 
...
case 'seq', % use the Statistically EQuivalent model:
   ...
   gamma= gammabar+Delgamma; 
    ...
    vvhcl= vtv.*cos(theta(p)-theta_tv)-vrv.*cos(theta(p)+theta_rv);
    avhcl= vvhcl/c;
    ...
    adopp= double(adrift) + double(avhcl) + double(asurf)*double(ns(p));
