...
function Band = set_channel_params(f0, B, channel_name, vtv, theta_tv, vtd_a, vrv, theta_rv, vrd_a, h0, ht0, d0, hr0)
...
c2=1600; % speed of sound in bottom [m/s] (>1500 for hard, < 1500 for soft)
cut=20; % do not consider arrivals whose strength is below that of direct arrival divided by cut

fmin=f0; % minimum frequency [Hz]
Band=B; % bandwidth [Hz]

df= Band/60; % frequency resolution [Hz], f_vec=fmin:df:fmax; 

dt= 1/df;
%dt= 50e-3; % time resolution [seconds]
%dt= 1/B; % time resolution [seconds] %注意此时dt==dtau
T_SS=2; % coherence time of the small-scale variations [seconds]
...
T_tot= 2*T_SS; % total duration of the simulated signal [seconds]
...
% drifting: 
vtd= vtd_a*cos((1/20)*(2*pi*t_tot_vec)-rand*2*pi); 
theta_td= linspace(rand*2*pi,rand*2*pi,length(t_tot_vec)); 

vrd= vrd_a*cos((1/5)*(2*pi*t_tot_vec)); 
theta_rd= rand*2*pi; 
